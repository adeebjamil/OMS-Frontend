...
export default function EvaluationsPage() {
  ...
  const loadUsers = async () => {
    try {
      ...
      const interns = (res.data.data || []).filter((u: UserOption) => u.role === 'intern');
      setUsers(interns);
    } catch (error) {
  ...
  const handleExportCSV = () => {
    ...
    const csvData = filteredEvals.map(ev => {
      const internName = (ev.internId && typeof ev.internId === 'object') ? ev.internId.name || '' : '';
      const internEmail = (ev.internId && typeof ev.internId === 'object') ? ev.internId.email || '' : '';
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-2">Review and track intern performance assessments</p>
        </div>
          ...
          <Card className="border-0 shadow-md">
            <CardContent className="pt-12 pb-12 text-center">
              ...
              <p className="text-sm text-gray-400">
                {currentUser?.role === 'admin'
                  ? 'Create performance evaluations for interns'
            ...
            <CardContent className="pt-6 overflow-y-auto flex-1 bg-white">
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  <div>
                    <label className="text-sm font-semibold text-gray-700 mb-2 block">Intern *</label>
                    <select
                      required
                ...
                <div className="border-t pt-4 space-y-4">
                  <div>
                    ...
                    <Textarea
                      required
                      placeholder="What does this intern do well? Highlight their key strengths..."
